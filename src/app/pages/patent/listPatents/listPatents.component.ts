...
export class ListPatentsComponent implements OnInit {
  ...
  checkStartedParking(){
    this.parkingService.existParkingOfUser(this.userId)
    .subscribe(data=>{
  ...
  finalize():void{
   this.parkingService.finishParking(this.userId)
   .subscribe({
     next:(data)=>{
       this.parking=data;
       this.startedParking= false;
       
       this.toastr.success('Se acredito el pago de '+this.parking.amount+'$', 'Pago acreditado!', {
         timeOut: 3000, positionClass: 'toast-top-center'
       });
  ...
  listAllPatents():void{
    this.patentService.getAll(this.tokenService.getIdUser())
    .subscribe(
      data => {
        this.patents = data;
      }
  ...
  deleteById(patent:Patent):void{
    if (confirm("¿Estás seguro de eliminar la patente "+ patent.patent + "?")) {
    this.patentService.delete(patent.id)
    .subscribe({
      next:() => {
